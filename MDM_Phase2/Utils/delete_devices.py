...
logger=setup_log(os.path.join(log_dir,f"{timestamp}_delete_devices.text"))

def delete_device():

    print("test initiated to delete device in test stage")
    logger.info("Test initiated to delete device in test stage")
        ...
        if 'models_v2' in config and 'imported_devices' in config:
            for i in config['models_v2']:
                ...
                if response.status_code == 204:
                    ...
                    #delete the models present in devices dict
