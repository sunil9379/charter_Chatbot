...
log_file = os.path.join(log_dir, f"{timestamp}_vendor_info.txt")
logger = setup_log(log_file, logger_name="file11")

def vendors_info():

    print("Test initiated for get vendor info")
    logger.info("Test initiated for get vendor info")
    ...
    try:
        ...
        if 'vendors' in config:
            ...
            for m in config['vendors']:
                model_url = url+ "v1/catalog/vendors/" + m
            ...
            print("Vendors not present in config.json")
            logger.error("Vendors not present in config.json")
        ...
        print("Test Completed for get vendor info")
        logger.info("Test completed for get vendor info")
