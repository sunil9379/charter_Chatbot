...
def get_firmware_info(modelname,firmware):
    ...
    for m in config["models_v2"]:
        ...
        firmware_url = f"{url}v2/catalog/models{modelname}/firmwares/{firmware}?referenceId={ref_id}"
        response = requests.get(url=firmware_url, headers=headers, verify=False)
